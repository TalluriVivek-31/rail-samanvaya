...
export async function resolveLocationIntelligence(
  ...
  const sKm = infraResult.startKm ?? 0;
  const eKm = infraResult.endKm ?? 0;
  ...
  return {
    ...
    section: {
      ...
      isCrossSection: infraResult.isCrossSection ?? false,
    ...
    line: {
      ...
      availableLines: infraResult.detectedLines || ['Main Line']
    },
    track: {
      requestedTrack,
      availableTracks: availableTracks.map((t: any) => t.trackName),
    ...
    locationRange: {
      ...
      affectedLengthKm: infraResult.affectedLengthKm ?? 0,
      affectedLengthMeters: infraResult.affectedLengthMeters ?? 0
    },
    assets: {
