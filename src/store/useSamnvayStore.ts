...
import { parseRailwayKm, detectCrossSectionSpans } from '../utils/railwayLocation';
    ...
    if (typeof window !== 'undefined' && window.localStorage) {
      ...
      if (saved) {
        ...
        return {
          ...getInitialState(),
          ...parsed,
          // CRITICAL: NEVER restore liveData from old localStorage sessions!
          // Live telemetry and timestamps must always start fresh dynamically.
          liveData: getInitialState().liveData,
...
export function useSamnvayStore() {
  ...
  const createRequest = useCallback((newReq: Omit<BlockRequest, 'id' | 'createdAt' | 'status' | 'engineer' | 'creatorRole'>) => {
    ...
    // Cross-Section Spans Analysis
    const startKm = typeof newReq.startKm === 'number' ? newReq.startKm : parseRailwayKm(newReq.startLocation);
    const endKm = typeof newReq.endKm === 'number' ? newReq.endKm : parseRailwayKm(newReq.endLocation);

    // Strict Railway Domain Validation (Section 28 of specification)
    if (!isNaN(startKm) && !isNaN(endKm) && startKm >= endKm) {
      setNotification(`Validation Error: startKm (${startKm}) must be strictly less than endKm (${endKm}).`, 'error');
      throw new Error(`INVALID_LOCATION_CHAINAGE: startKm (${startKm}) must be strictly less than endKm (${endKm})`);
    }
    if (newReq.duration <= 0) {
      setNotification(`Validation Error: duration must be greater than 0 minutes.`, 'error');
      throw new Error(`INVALID_DURATION: Requisition duration must be greater than 0 minutes (got ${newReq.duration})`);
    }

    const crossAnalysis = (!isNaN(startKm) && !isNaN(endKm)) ? detectCrossSectionSpans(startKm, endKm) : undefined;

    // Multi-Department Initial Execution Statuses
    const involvedDepts: Department[] = [newReq.department, ...(newReq.otherDepartmentsInvolved || [])];
    const uniqueDepts = Array.from(new Set(involvedDepts));
    const initialDeptStatuses: DepartmentExecutionStatus[] = uniqueDepts.map(dept => ({
      department: dept,
      status: 'PENDING',
      workDescription: dept === newReq.department ? newReq.work : `${dept} coordinated support & isolation`,
    ...
    const fullRequest: BlockRequest = {
      ...
      status: 'Submitted',
      planned_duration: newReq.planned_duration || newReq.duration,
      planned_start: newReq.planned_start || newReq.preferredStartTime || newReq.preferredTime,
      planned_end: newReq.planned_end || newReq.preferredEndTime,
      startKm: !isNaN(startKm) ? startKm : newReq.startKm,
      endKm: !isNaN(endKm) ? endKm : newReq.endKm,
      isCrossSection: crossAnalysis?.isCrossSection || newReq.isCrossSection,
      crossSections: crossAnalysis?.sections || newReq.crossSections,
      departmentExecutionStatuses: initialDeptStatuses,
      statusHistory: [
        {
          status: 'Submitted',
          timestamp: timeNow,
          ...
          role: globalState.currentUser.role,
          remarks: `Requisition created by ${globalState.currentUser.name} (${globalState.currentUser.role})${crossAnalysis?.isCrossSection ? `. CROSS-SECTION SPAN: Spans ${crossAnalysis.sections.join(' & ')} across boundary.` : ''}`
        }
      ],
      createdAt: timeNow,
    };

    if (!fullRequest.planned_end && fullRequest.planned_start && fullRequest.planned_duration) {
      const [sH, sM] = fullRequest.planned_start.split(':').map(Number);
      const endTotalMins = ((sH || 0) * 60 + (sM || 0) + fullRequest.planned_duration) % 1440;
      const eH = Math.floor(endTotalMins / 60);
      const eM = endTotalMins % 60;
      fullRequest.planned_end = `${String(eH).padStart(2, '0')}:${String(eM).padStart(2, '0')}`;
    }
  ...
  const transitionBlockStatus = useCallback((
    ...
    // Duplicate approval prevention (Test 23 & Section 21)
    if (targetStatus === 'Approved' && req.status === 'Approved') {
      setNotification(`Requisition ${requestId} is already Approved.`, 'warning');
      return { success: false, message: 'ALREADY_APPROVED: Requisition is already Approved' };
    }
    ...
    const currentStatusKey = req.status as string;
    const allowed = ALLOWED_TRANSITIONS[currentStatusKey];
    if (allowed && !allowed.includes(targetStatus)) {
      const msg = `Invalid state transition: Cannot transition from '${req.status}' to '${targetStatus}'.`;
  ...
  const advanceExecutionStep = useCallback((targetRequestId?: string): { success: boolean; message?: string } => {
    let steps = [...globalState.executionSteps];
    const activeReq = targetRequestId
    ...
    if (targetRequestId && activeReq && (activeReq.status === 'Approved' || steps.every(s => s.status === 'COMPLETED'))) {
      steps = INITIAL_EXECUTION_STEPS.map(s => ({ ...s, status: 'PENDING' as const, timestamp: undefined, confirmedBy: undefined }));
    }

    let inProgressIndex = steps.findIndex(s => s.status === 'IN_PROGRESS');
    ...
    if (inProgressIndex === -1) {
      ...
      if (activeReq) {
        ...
        updatedRequests = globalState.requests.map(r => r.id === activeReq.id ? { 
          ...
          planned_start: r.planned_start || (r.allocatedWindow ? r.allocatedWindow.startTime : r.preferredTime),
          planned_duration: r.planned_duration || r.duration,
          blockMemoNumber: r.blockMemoNumber || `MEMO-BZA-${Math.floor(1000 + Math.random() * 9000)}`,
          statusHistory: newHistory
        } : r);
      }
    ...
    // Strict Rule: Step 5 (Maintenance Completed) -> Step 6 (Block Released)
    // ONLY Section Controller / Operating Control can release possession
    if (inProgressIndex === 4) {
      if (!['MASTER', 'COA / Operations', 'Section Controller'].includes(globalState.currentUser.role)) {
        const msg = 'Access Denied: Under Indian Railways G&SR Chapter XV, only Section Controller / Operating Control Authority can authorize Block Release.';
        setNotification(msg, 'error');
        return { success: false, message: msg };
      }

      // Multi-Department Verification prior to Block Release (Section 19 of specification)
      if (activeReq?.departmentExecutionStatuses && activeReq.departmentExecutionStatuses.length > 0) {
        const pendingDept = activeReq.departmentExecutionStatuses.find(d => d.status !== 'COMPLETED' && d.status !== 'PARTIALLY_COMPLETED');
        if (pendingDept) {
          const msg = `Cannot release block: Department '${pendingDept.department}' execution status is '${pendingDept.status}'. All participating departments must sign off and complete work before block release.`;
          setNotification(msg, 'error');
          return { success: false, message: msg };
        }
      }
    ...
    if (inProgressIndex < steps.length - 1) {
      ...
      steps[inProgressIndex + 1].timestamp = timeNow;
      ...
      let updatedRequests = globalState.requests;
      if (activeReq) {
        // If transitioning into Step 5 (Maintenance Completed), record default completion report if not provided
        let completionRep = activeReq.completionReport;
        if (inProgressIndex === 3 && !completionRep) {
          const plannedDur = activeReq.planned_duration || activeReq.duration || 60;
          completionRep = {
            id: `REP-${Date.now()}`,
            requestId: activeReq.id,
            blockId: activeReq.blockMemoNumber,
            status: 'WORK_COMPLETED',
            completedAt: timeNow,
            actualWorkCompletion: timeNow,
            submittedBy: globalState.currentUser.name,
            userRole: globalState.currentUser.role,
            department: activeReq.department,
            plannedDurationMinutes: plannedDur,
            actualDurationMinutes: plannedDur,
            varianceMinutes: 0,
            workAccomplishedSummary: 'Physical track maintenance completed per standard engineering specification. Track clearance and gauge verified fit.',
            infrastructureCondition: 'RESTORATION_PENDING'
          };
        }

        // If transitioning into Step 6 (Block Released), record actual_end and calculate actual_duration & variance
        const isReleasing = inProgressIndex === 4;
        const isWorkCompleting = inProgressIndex === 3;
        const actualWorkComp = isWorkCompleting ? timeNow : activeReq.actual_work_completion;
        const actualBlockRel = isReleasing ? timeNow : activeReq.actual_block_release;
        const actualEnd = isReleasing ? timeNow : activeReq.actual_end;
        const actualDur = isReleasing ? (activeReq.actual_duration || activeReq.duration) : activeReq.actual_duration;
        const plannedDur = activeReq.planned_duration || activeReq.duration;
        const variance = actualDur ? (actualDur - plannedDur) : activeReq.duration_variance;
        ...
        const newHistory: StatusHistoryEntry[] = [
          ...(activeReq.statusHistory || []),
          {
            ...
            actor: globalState.currentUser.name,
            role: globalState.currentUser.role,
            remarks: `Advanced to ${steps[inProgressIndex + 1].title}${isReleasing ? ' (Block Released by Operating Control)' : ''}`
          }
        ];
        ...
        updatedRequests = globalState.requests.map(r => r.id === activeReq.id ? { 
          ...r, 
          status: nextTargetStatus,
          completionReport: completionRep,
          actual_work_completion: actualWorkComp,
          actual_block_release: actualBlockRel,
          actual_end: actualEnd,
          actual_duration: actualDur,
          duration_variance: variance,
          operationalRestriction: r.operationalRestriction || (isReleasing ? {
            id: `TSR-DEFAULT-${Date.now()}`,
            requestId: r.id,
            type: 'NORMAL',
            normalSectionSpeedKmph: 130,
            effectiveFrom: timeNow,
            authorizedBy: globalState.currentUser.name,
            authorizerRole: globalState.currentUser.role,
            reason: 'Track certified fit for normal sectional speed (130 km/h).',
      ...
      if (activeReq) {
        ...
        const newHistory = [
          ...(activeReq.statusHistory || []),
          {
            ...
            actor: globalState.currentUser.name,
            role: globalState.currentUser.role,
            remarks: 'Fitness Certificate endorsed. Track possession released & closed in master ledger.'
          }
        ];
        updatedRequests = globalState.requests.map(r => r.id === activeReq.id ? { 
          ...r, 
  ...
  // SUBMIT FORMAL COMPLETION REPORT
  const submitCompletionReport = useCallback((
    ...
    const timeNow = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' }) + ' IST';
    const plannedDuration = req.planned_duration || req.duration || 60;
    const actualDuration = reportData.actualDurationMinutes || plannedDuration;
    ...
    // Completed work defaults to RESTORATION_PENDING until formal authorized restoration occurs
    let condition: InfrastructureCondition = 'RESTORATION_PENDING';
    if (reportData.status === 'PARTIALLY_COMPLETED') {
    ...
    const actualWorkComp = (reportData as any).actualWorkCompletion || timeNow;
    const remainingStartKm = (reportData as any).remainingStartKm;
    const remainingEndKm = (reportData as any).remainingEndKm;

    const report: CompletionReport = {
      id: `REP-${Date.now()}`,
      requestId,
      blockId: req.blockMemoNumber,
      status: reportData.status,
      completedAt: timeNow,
      actualWorkCompletion: actualWorkComp,
      submittedBy: globalState.currentUser.name,
      userRole: globalState.currentUser.role,
      ...
      workAccomplishedSummary: reportData.workAccomplishedSummary,
      incompletionReasonCategory: reportData.incompletionReasonCategory,
      incompletionDetails: reportData.incompletionDetails,
      remainingWork: reportData.remainingWork,
      remainingLocationKm: reportData.remainingLocationKm,
      remainingStartKm,
      remainingEndKm,
      infrastructureCondition: condition,
      continuationRequired: Boolean(reportData.continuationRequired),
      continuationRequestedDurationMinutes: reportData.continuationRequestedDurationMinutes,
      severity: reportData.severity || 'LOW'
    };

    const updatedDeptStatuses = (req.departmentExecutionStatuses || []).map(ds => {
      if (ds.department === req.department) {
        return {
          ...ds,
          status: (reportData.status === 'WORK_COMPLETED' || (reportData.status as any) === 'COMPLETED') ? ('COMPLETED' as const) : ('PARTIALLY_COMPLETED' as const),
          completedAt: timeNow,
          signedOffBy: globalState.currentUser.name,
          remarks: reportData.workAccomplishedSummary
        };
      }
      return ds;
    });
    ...
    if (steps[3]?.status === 'IN_PROGRESS') {
      steps[3].status = 'COMPLETED';
      steps[4].status = 'IN_PROGRESS';
      steps[4].timestamp = timeNow;
      steps[4].confirmedBy = `${globalState.currentUser.name} (${globalState.currentUser.role})`;
    }
    ...
    if (reportData.continuationRequired || targetStatus === 'PARTIALLY_COMPLETED') {
      ...
      const continuationReq: BlockRequest = {
        ...
        endKm: remainingEndKm ?? req.endKm,
        work: `[Continuation] ${remWork}`,
        ...
        affectedTracks: req.affectedTracks,
        trafficBlockRequired: req.trafficBlockRequired,
        ...
        statusHistory: [
          {
            ...
            actor: globalState.currentUser.name,
            role: globalState.currentUser.role,
            remarks: `Chained Continuation Requisition generated from parent ${requestId}. Queued for planning. (Never auto-granted).`
          }
        ],
        createdAt: timeNow
      };
      ...
      updatedRequests = [
        continuationReq,
        ...updatedRequests.map(r => r.id === requestId ? {
          ...r,
          continuationBlockIds: [...(r.continuationBlockIds || []), chainedContinuationId!]
        } : r)
      ];
    }
    ...
    logAudit(
      ...
      requestId,
      reportData.status,
      `Completion report filed by ${globalState.currentUser.name}: ${reportData.status} (Variance: ${variance >= 0 ? '+' : ''}${variance}m). ${chainedContinuationId ? `Chained continuation ${chainedContinuationId} created.` : ''}`
    );

    notify();
    setNotification(
      `Completion report recorded: ${reportData.status}${chainedContinuationId ? `. Continuation requisition ${chainedContinuationId} queued for planning.` : ''}`,
      reportData.status === 'WORK_COMPLETED' ? 'success' : 'warning'
    );

    return { success: true, reportId: report.id, continuationId: chainedContinuationId };
  }, [logAudit, setNotification]);

  // REQUEST CONTINUATION BLOCK (NEVER AUTO-GRANTED)
  const requestContinuationBlock = useCallback((
    ...
    const parentReq = globalState.requests.find(r => r.id === parentRequestId);
    if (!parentReq) {
      setNotification(`Error: Parent requisition ${parentRequestId} not found.`, 'error');
      return { success: false };
    }

    const timeNow = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' }) + ' IST';
    const contId = `BR-${1020 + globalState.requests.length + 1}`;

    const newReq: BlockRequest = {
      id: contId,
      continuationOfBlockId: parentRequestId,
      parentRequirementId: parentRequestId,
      department: data.department || parentReq.department,
      engineer: globalState.currentUser.name,
      creatorRole: globalState.currentUser.role,
      section: parentReq.section,
      startLocation: data.remainingLocationKm || parentReq.startLocation,
      endLocation: parentReq.endLocation,
      startKm: parentReq.startKm,
      endKm: parentReq.endKm,
      work: `[Continuation] ${data.remainingWork}`,
      workCategory: parentReq.workCategory,
      date: data.preferredDate || parentReq.date,
      preferredTime: data.preferredTime || '02:00',
      duration: data.requestedDurationMinutes,
      planned_duration: data.requestedDurationMinutes,
      priority: parentReq.priority,
      risk: parentReq.risk,
      status: 'Submitted',
      reason: `Continuation requisition for incomplete work on ${parentRequestId}. Reason: ${data.reason}`,
      safetyRequirements: parentReq.safetyRequirements || [],
      resourcesRequired: parentReq.resourcesRequired || [],
      priorityScore: parentReq.priorityScore,
      priorityBreakdown: parentReq.priorityBreakdown,
      affectedTracks: parentReq.affectedTracks,
      statusHistory: [
        {
          status: 'Submitted',
          timestamp: timeNow,
          actor: globalState.currentUser.name,
          role: globalState.currentUser.role,
          remarks: `Continuation Requisition created from parent ${parentRequestId}. Queued for full approval workflow.`
        }
      ],
      createdAt: timeNow
    };
    ...
    const updatedRequests = [
      newReq,
      ...globalState.requests.map(r => r.id === parentRequestId ? {
        ...r,
        continuationBlockIds: [...(r.continuationBlockIds || []), contId]
      } : r)
    ];
    ...
    logAudit(
      'Continuation Requisition Submitted',
      contId,
      'Submitted',
      `Continuation requisition ${contId} created linked to parent ${parentRequestId} (${data.requestedDurationMinutes}m). Mandatory Rule: Never auto-granted.`
    );

    notify();
    setNotification(`Continuation block ${contId} created and queued for technical review. (Continuation blocks are never auto-granted).`, 'success');
    return { success: true, continuationId: contId };
  }, [logAudit, setNotification]);

  // RECORD OPERATIONAL SPEED RESTRICTION / TRACK FIT
  const recordOperationalRestriction = useCallback((
    requestId: string,
    data: {
      type: OperationalRestrictionType;
      speedKmph?: number;
      normalSectionSpeedKmph?: number;
      reason: string;
      remarks?: string;
    ...
    const timeNow = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' }) + ' IST';
    const normalSpeed = data.normalSectionSpeedKmph || 130;

    if (data.type === 'RESTRICTED') {
      if (!data.speedKmph || data.speedKmph < 10 || data.speedKmph >= normalSpeed) {
        setNotification(`Error: Speed restriction must be between 10 km/h and ${normalSpeed} km/h. Cannot arbitrarily hardcode.`, 'error');
        return { success: false };
      }
    }

    const restriction: OperationalRestriction = {
      id: `TSR-${Date.now()}`,
      requestId,
      blockId: req.blockMemoNumber,
      type: data.type,
      speedKmph: data.type === 'RESTRICTED' ? data.speedKmph : undefined,
      normalSectionSpeedKmph: normalSpeed,
      effectiveFrom: timeNow,
      authorizedBy: globalState.currentUser.name,
      authorizerRole: globalState.currentUser.role,
      reason: data.reason || (data.type === 'RESTRICTED' ? 'Temporary Speed Restriction' : 'Track fit for normal speed'),
      remarks: data.remarks,
      cautionOrderIssued: Boolean(data.cautionOrderIssued),
      cautionOrderNumber: data.cautionOrderNumber || (data.type === 'RESTRICTED' ? `CO-BZA-${Math.floor(1000 + Math.random() * 9000)}` : undefined)
    };

    const updatedSections = globalState.sections.map(s => {
      if (s.id === req.section || (req.crossSections && req.crossSections.includes(s.id))) {
        if (data.type === 'RESTRICTED' && data.speedKmph) {
          return {
            ...s,
        ...
        } else if (data.type === 'NORMAL' || data.type === 'NORMAL_RESTORED') {
          return {
            ...s,
    ...
    logAudit(
      'Operational Restriction Recorded',
      requestId,
      data.type,
      `Track fit recorded as ${data.type} ${data.type === 'RESTRICTED' ? `(TSR: ${data.speedKmph} km/h)` : '(Normal Sectional Speed)'} by ${globalState.currentUser.name}. Reason: ${data.reason}`
    );

    notify();
    setNotification(`Track operational condition recorded: ${data.type} ${data.type === 'RESTRICTED' ? `(${data.speedKmph} km/h)` : '(Normal Speed)'}.`, 'success');
    return { success: true, restrictionId: restriction.id };
  }, [logAudit, setNotification]);

  // UPDATE MULTI-DEPARTMENT EXECUTION STATUS
  const updateDepartmentExecutionStatus = useCallback((
    requestId: string,
    department: Department,
    ...
    let updated = existing.map(d => {
      if (d.department === department) {
        return {
          ...d,
          status,
          completedAt: (status === 'COMPLETED' || status === 'PARTIALLY_COMPLETED') ? timeNow : d.completedAt,
          signedOffBy: `${globalState.currentUser.name} (${globalState.currentUser.role})`,
    ...
    if (!existing.some(d => d.department === department)) {
      updated.push({
        department,
        status,
        workDescription: `${department} on-site execution`,
        isolationVerified: true,
        restorationVerified: status === 'COMPLETED',
        completedAt: status === 'COMPLETED' ? timeNow : undefined,
        signedOffBy: `${globalState.currentUser.name} (${globalState.currentUser.role})`,
        remarks
      });
    }
    ...
    const updatedRequests = globalState.requests.map(r => r.id === requestId ? {
      ...r,
      departmentExecutionStatuses: updated
    } : r);
    ...
    logAudit(
      'Department Execution Status Updated',
      requestId,
      status,
      `${department} execution updated to ${status} by ${globalState.currentUser.name}.`
    );

    notify();
    setNotification(`${department} execution status marked as ${status}.`, 'info');
    return { success: true };
  }, [logAudit, setNotification]);

  // RAILWAY-COMPLIANT AUDITED CANCELLATION (NEVER SILENTLY DELETE)
  const cancelBlockRequest = useCallback((
    requestId: string,
    reason: string
  ) => {
    const req = globalState.requests.find(r => r.id === requestId);
    ...
    const timeNow = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' }) + ' IST';
    const cancellation = {
      cancelledBy: globalState.currentUser.name,
      role: globalState.currentUser.role,
      timestamp: timeNow,
      reason
    };
    ...
    const newHistory: StatusHistoryEntry[] = [
      ...(req.statusHistory || []),
      {
        status: 'Unsafe / Cancelled' as BlockStatus,
        timestamp: timeNow,
        actor: globalState.currentUser.name,
        role: globalState.currentUser.role,
        remarks: `Block cancelled by ${globalState.currentUser.name} (${globalState.currentUser.role}). Reason: ${reason}`
      }
    ];
    ...
    logAudit(
      'Maintenance Block Cancelled',
      requestId,
      'Unsafe / Cancelled',
      `Block ${requestId} marked as Unsafe / Cancelled with full audit history preserved. Reason: "${reason}"`
    );

    notify();
    setNotification(`Block ${requestId} cancelled and logged into permanent audit ledger.`, 'warning');
    return { success: true };
  }, [logAudit, setNotification]);

  // FORMAL INFRASTRUCTURE CONDITION RESTORATION (Section 3, 4 of specification)
  // Only Section Controller / Operating Control Authority (COA / Operations) or MASTER can authorize
  const recordRestoration = useCallback((
    requestId: string,
    newCondition: InfrastructureCondition,
    verificationRemarks: string,
    restrictionReference?: string
  ) => {
    const isOperatingControl = globalState.currentUser.role === 'COA / Operations' || 
                               globalState.currentUser.role === 'Section Controller' || 
                               globalState.currentUser.role === 'MASTER';
    if (!isOperatingControl) {
      setNotification(`Forbidden: Only Section Controller / Operating Control can authorize infrastructure restoration under G&SR 15.06.`, 'error');
      return { success: false, error: 'UNAUTHORIZED_RESTORATION_AUTHORITY' };
    }
    ...
    const timeNow = new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' }) + ' IST';
    const restorationRecord: RestorationRecord = {
      id: `REST-${Date.now()}`,
      requestId,
      blockId: req.blockMemoNumber,
      restoredBy: globalState.currentUser.name,
      userRole: globalState.currentUser.role,
      restoredAt: timeNow,
      previousCondition: req.infrastructureCondition || 'RESTORATION_PENDING',
      newCondition,
      verificationRemarks,
        ...
        return {
          ...
          statusHistory: [
            ...(r.statusHistory || []),
            {
              ...
              actor: globalState.currentUser.name,
              role: globalState.currentUser.role,
              remarks: `Infrastructure Condition restored to '${newCondition}' by ${globalState.currentUser.name} (${globalState.currentUser.role}). Verification: ${verificationRemarks}`
            }
          ]
        };
      }
  ...
  // Preserves original system recommendation while recording authorized human modification
  const recordManualOverride = useCallback((
    requestId: string,
    arg2: string | { startTime: string; endTime: string; durationMinutes?: number },
    arg3?: string,
    arg4?: string
  ) => {
    const isAuthorized = globalState.currentUser.role === 'Planning Officer' || 
                         globalState.currentUser.role === 'COA / Operations' || 
                         globalState.currentUser.role === 'Section Controller' ||
                         globalState.currentUser.role === 'MASTER';
    if (!isAuthorized) {
      setNotification(`Forbidden: Role '${globalState.currentUser.role}' cannot override system recommendations.`, 'error');
      return { success: false, error: 'UNAUTHORIZED_OVERRIDE' };
    }
    ...
    let modifiedStartTime: string;
    let modifiedEndTime: string;
    let reason: string;
    let modifiedDuration: number;

    if (typeof arg2 === 'object') {
      modifiedStartTime = arg2.startTime;
      modifiedEndTime = arg2.endTime;
      reason = arg3 || 'Operational override';
      modifiedDuration = arg2.durationMinutes || 120;
    } else {
      modifiedStartTime = arg2;
      modifiedEndTime = arg3 || '04:00';
      reason = arg4 || 'Operational override';
      const [mStartH, mStartM] = modifiedStartTime.split(':').map(Number);
      const [mEndH, mEndM] = modifiedEndTime.split(':').map(Number);
      modifiedDuration = ((mEndH || 0) * 60 + (mEndM || 0)) - ((mStartH || 0) * 60 + (mStartM || 0));
    }
    ...
    const manualOverride: ManualOverride = {
      ...
      originalRecommendation: {
        startTime: req.allocatedWindow?.startTime || '04:30',
        endTime: req.allocatedWindow?.endTime || '06:30',
        durationMinutes: req.duration || 120
      },
      modifiedValues: {
        startTime: modifiedStartTime,
        endTime: modifiedEndTime,
        durationMinutes: modifiedDuration
      },
      modifiedBy: globalState.currentUser.name,
      modifiedRole: globalState.currentUser.role,
      modifiedAt: timeNow,
      reason
    };
      ...
      if (r.id === requestId) {
        return {
          ...
          allocatedWindow: {
            startTime: modifiedStartTime,
            endTime: modifiedEndTime,
            safetyBufferBefore: r.allocatedWindow?.safetyBufferBefore ?? 15,
            safetyBufferAfter: r.allocatedWindow?.safetyBufferAfter ?? 15
          },
          ...
          statusHistory: [
            ...(r.statusHistory || []),
            {
              ...
              actor: globalState.currentUser.name,
              role: globalState.currentUser.role,
              remarks: `Manual Planning Override: Window modified to ${modifiedStartTime}–${modifiedEndTime} (${modifiedDuration}m). Original recommendation (${manualOverride.originalRecommendation.startTime}–${manualOverride.originalRecommendation.endTime}) preserved in ledger. Reason: ${reason}`
            }
          ]
        };
      }
    ...
    globalState = { ...globalState, requests: updatedRequests };
    notify();
    logAudit('MANUAL_OVERRIDE', requestId, 'MODIFIED', `Window modified to ${modifiedStartTime}–${modifiedEndTime}. Reason: ${reason}`);
    setNotification(`Manual override applied for ${requestId}: ${modifiedStartTime}–${modifiedEndTime}.`, 'info');
    return { success: true, manualOverride };
  }, [logAudit, setNotification]);

  // CHECK OVERDUE COMPLETION REPORTS (Section 6, 7 of specification)
  const checkOverdueCompletionReports = useCallback((currentTimeStr?: string) => {
    const now = new Date();
    ...
    const updatedRequests = globalState.requests.map(r => {
      ...
      if (r.planned_end) {
        const [endH, endM] = r.planned_end.split(':').map(Number);
        endMins = (endH || 0) * 60 + (endM || 0);
      } else if (r.planned_start || r.preferredTime) {
        const startTime = r.planned_start || r.preferredTime;
        const [sH, sM] = startTime.split(':').map(Number);
        endMins = ((sH || 0) * 60 + (sM || 0) + (r.planned_duration || r.duration || 60)) % 1440;
      }

      if (
        (r.status === 'Block Started' || r.status === 'Work in Progress' || r.status === 'Block Release Requested') &&
        !r.completionReport &&
        endMins !== null &&
        currMins >= endMins
      ) {
        ...
        return {
          ...r,
          isOverdueCompletionReport: true,
          completionReportStatus: 'COMPLETION_REPORT_OVERDUE' as const
        };
      }
      return r;
    });
  ...
  const updateStationBoard = useCallback((stationCode: string, entries: StationBoardEntry[], source: DataSource) => {
    if (source === 'UNAVAILABLE' || !entries || entries.length === 0) {
      globalState = {
        ...globalState,
        liveData: {
          ...globalState.liveData,
